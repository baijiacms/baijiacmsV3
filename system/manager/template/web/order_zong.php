...
<table class="table table-striped table-bordered table-hover">
		...
		<tr>
				...
				<td>
					<textarea style="height:50px;" class="span7" name="zong_remark" cols="70" ><?php  echo $order['zong_remark']?></textarea>		<button type="submit" class="btn-success" name="save_zong_remark"  value="save_zong_remark">保存备注</button>
				</td>	
					</table>
			...
			<tr>
				...
				<td>
					...
					<?php  if(empty($order['zong_status'])) { ?>
				...
				<?php  } ?>
					...
					<?php  if($order['zong_status'] == 2) { ?>
						<button type="submit" class="btn btn-danger span2" name="zong_cancelsend" onclick="return confirm('取消发货此订单吗？'); return false;" value="zong_cancelsend">取消发货</button>
         <?php  } ?>
					...
					<?php  if($order['zong_status'] ==2) { ?>
					<button type="submit" class="btn btn-success span2" onclick="return confirm('确认完成此订单吗？'); return false;" name="zong_finish" value="zong_finish">完成订单</button>
				<?php  } ?>
					
					
					
					
					<?php  if($order['status']==1&&($order['zong_status']==0||$order['zong_status']==-2)) { ?>
					<button type="button" class="btn span2" name="zong_returnmoney_pre" value="zong_returnmoney_pre" data-toggle="modal" data-target="#modal-zong_returnmoney">订单退款</button>
					<?php  } ?>
					
					
					<?php  if(empty($order['status'])&&$order['zong_status']==0) { ?>
					<button type="submit" class="btn span2" name="zong_close" onclick="return confirm('永久关闭此订单吗？'); return false;" value="zong_close">关闭订单</button>
					<?php  } ?>
      	...
      	<div class="form-group">
      		  ...
      		  <div class="form-group">
										...
										<div class="col-sm-9">
												...
												<input type="text" name="returnmoney" class="span5" value="<?php echo ($order['zong_goodsprice']+$order['dispatchprice']);?>" />
											</div>
									</div>
      	
      </div>
      <div class="modal-footer">
      	<button type="submit" class="btn btn-primary" name="zong_returnmoney" value="zong_returnmoney"  onclick="return confirm('确定退款此订单吗？'); return false;">确认退款</button>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
      	<button type="submit" class="btn btn-primary" name="zong_confirmsend" value="yes">确认发货</button>
