<?php
        ...
        $category = mysqld_selectall("SELECT * FROM " . table('shop_category') . " where deleted=0 and is_system=1 ORDER BY parentid ASC, displayorder DESC", array(), 'id');
        if (!empty($category)) {
            $children = '';
        ...
        if ($operation == 'post') {
            $id = intval($_GP['id']);
            if (!empty($id)) {
                $item = mysqld_select("SELECT * FROM " . table('shop_goods') . " WHERE id = :id  and is_system=1", array(':id' => $id));
                if (empty($item)) {
            ...
            if (checksubmit('submit')) {
                ...
                {
                   $goodssn_goods = mysqld_select("SELECT id FROM " . table('shop_goods') . " where deleted=0 and is_system=1 and goodssn=:goodssn limit 1", array(':goodssn'=>$_GP['goodssn']));
                ...
                $data = array(
                 		'goodsFHprice' => $_GP['goodsFHprice'],
                		'goodsFHtype' => intval($_GP['goodsFHtype']),
                ...
                if (empty($id)) {
                    ...
                    unset($data['createtime']);
                     unset($data['sales']);
                    mysqld_update('shop_goods', $data, array('id' => $id,'is_system' => 1));
                }
  ...
  $tspecitems = mysqld_select("SELECT id FROM " . table('shop_goods_spec_item') . " WHERE id = :id", array(':id' => $get_item_id));
                ...
                //总库存
                if ($totalstocks > 0) {
                    mysqld_update("shop_goods", array("total" => $totalstocks), array("id" => $id,'is_system' => 1));
                }
            ...
            $list = mysqld_selectall("SELECT * FROM " . table('shop_goods') . " WHERE  deleted=0 and is_system=1 $condition ORDER BY status DESC, displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
            $total = mysqld_selectcolumn('SELECT COUNT(*) FROM ' . table('shop_goods') . " WHERE deleted=0 and is_system=1 $condition");
            $pager = pagination($total, $pindex, $psize);
             include page('goods_list');
        } elseif ($operation == 'delete') {
            $id = intval($_GP['id']);
            $row = mysqld_select("SELECT id, thumb FROM " . table('shop_goods') . " WHERE id = :id and is_system=1 ", array(':id' => $id));
            if (empty($row['id'])) {
                message('抱歉，商品不存在或是已经被删除！');
            }
           	$shop_order_goods = mysqld_selectcolumn("SELECT count(*) FROM " . table('shop_order_goods')." shop_category where  goodsid=:goodsid",array(":goodsid"=>$row['id']));
        	  if($shop_order_goods>0)
      		  {  
          	  mysqld_update("shop_goods", array("deleted" => 1), array('id' => $id,'is_system' => 1));
            }else
            {
           	 mysqld_delete("shop_goods",  array('id' => $id,'is_system' => 1));
            }
