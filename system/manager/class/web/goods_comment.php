<?php
          ...
          if ($operation == 'add') {
             ...
             if (empty($shop_goods['id'])) {
           	...
           	if(empty($_GP['rate']))
           	{
           	 ...
           	 mysqld_insert('shop_goods_comment', array('is_system'=>1,'isenable'=>1,'beid'=>0,'createtime'=>time(),'comment_nickname'=> $_GP['comment_nickname'],'rate'=> $_GP['rate'],'ordersn' => '','optionname'=>'','goodsid'=>$shop_goods['id'],'comment' => $_GP['rsreson'],'orderid' =>'', 'openid' => ''));
 ...
 $shop_goods = mysqld_select("SELECT * FROM " . table('shop_goods') . " WHERE id = :id  ", array(':id' => $_GP['goodsid']));
        ...
        if ($operation == 'check') {
            ...
            $list = mysqld_selectall("SELECT comment.*,member.nickname,member.mobile,shop_goods.title FROM " . table('shop_goods_comment') . " comment  left join " . table('member') . " member on comment.openid=member.openid   left join " . table('shop_goods') . " shop_goods on shop_goods.id=comment.goodsid where comment.is_system=1 and isenable=0 ORDER BY comment.createtime   ");
        ...
        if ($operation == 'enable') {
            ...
            mysqld_update("shop_goods_comment",array('isenable'=>1),  array('id' => $id));
