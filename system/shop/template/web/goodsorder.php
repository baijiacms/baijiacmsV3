        ...
        <td> <?php  if(!empty($goods['optionname'])) { ?><?php  echo $goods['optionname'];?><?php  } ?></td>
				 ...
				 <?php  if($order['be_status'] == 2) { ?><span class="label label-warning">待收货</span><?php  } ?>
					...
					<?php  if($goods['status'] == 1&&$goods['restatus'] ==0) { ?><span class="label label-success" >已收货</span><?php  } ?>
						...
						<?php  if($goods['status'] == -5) { ?><span class="label label-success">已退货</span><?php  } ?>
							<?php  if($goods['status']==-5) { ?>		<?php  if($goods['be_return_money'] ==2) { ?><?php  if(empty($goods['returnmoneytype'])) { ?>退款到余额：<?php  echo $goods['returnmoney']; ?>元<?php  }else{ ?>已退款：<?php  echo $goods['returnmoney']; ?>元<?php  }?><?php  }else{ ?><span class="label label-danger" >等待退款</span>	<?php  } ?><?php  } ?>
			...
			<tr>
				...
				<td>
					...
					<button type="submit" class="btn span2  btn-danger" name="all_returnmoney" value="all_returnmoney" onclick="return confirm('确认进行退货吗？'); return false;">确认退货</button>
					<?php  } ?>
