<?php
			...
			if( $operation=='uninstall')
			{
				...
				{ 
					if(($file!=".") AND ($file!="..")) 
					{
						...
						$item = mysqld_select("SELECT * FROM " . table('dispatch') . " WHERE enabled=1 and code = :code and is_system=1", array(':code' => $file));
						if(empty($item['code']))
						{
		...
		include page('manager_dispatch');
		exit;
				       
				       	
				 }
				        
				       	$modules = mysqld_selectall("SELECT * FROM " . table('dispatch') . " WHERE enabled=1  and is_system=1");
