...
defined('SYSTEM_IN') or exit('Access Denied');
        	...
        	$rule = mysqld_select('SELECT * FROM '.table('weixin_rule')." WHERE  id = :id and beid=:beid" , array(':id' =>intval($_GP['id']),':beid'=>$_CMS['beid']));
    			...
    			if(checksubmit())
    			{
                    ...
                    $thumb = $upload['path'];
    					...
    					$data=array('beid'=>$_CMS['beid'],'title'=>$_GP['title'],'ruletype'=>$_GP['ruletype'],'keywords'=>$_GP['keywords'],'thumb'=>$thumb,'description'=>$_GP['description'],'url'=>$_GP['url']);
        ...
        $list=mysqld_selectall('SELECT * FROM '.table('weixin_rule')." where beid=:beid",array(':beid'=>$_CMS['beid']));
			include page('rule');